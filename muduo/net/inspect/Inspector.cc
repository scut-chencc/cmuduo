      ...
      processInspector_(new ProcessInspector)
{
  ...
  // 这样子做法是为了防止竞态问题，联系Inspector_test.cc里的注释
  ...
  loop->runAfter(0.1, boost::bind(&Inspector::start, this)); // little race condition,时间可以设置长点，这时构造函数已经执行完了
}
...
void Inspector::add(const string& module,//往commands_和help_里添加记录
                    const string& command,
                    const Callback& cb,
                    const string& help)
{
  MutexLockGuard lock(mutex_);//registerCommands()而执行到这个函数的，这时只有主线程访问，不用设置互斥量
...
{
  server_.start();//服务器启动，一旦客户端发起请求，就会回调函数OnRequest
}
...
void Inspector::onRequest(const HttpRequest& req, HttpResponse* resp)
{
  ...
  {
    string result;
    MutexLockGuard lock(mutex_);//如果http服务器的线程只有一个的话，则在onRequest函数里的列表也不需要保护，暴露一个监控接口也没必要多线程
    // 遍历helps 
