                      ...
                      public boost::enable_shared_from_this<TcpConnection>//跟tcpconnection的生存期管理有关系
{
 ...
 private:
  ...
  WriteCompleteCallback writeCompleteCallback_;		// 数据发送完毕回调函数，即(网络库)所有的用户数据都已拷贝到内核缓冲区时回调该函数 ，通知上层程序，以便让它发送更多的数据，大流量应用程序才需要
  ...
  boost::any context_;			// 表示一个连接对象，绑定一个未知类型的上下文对象,因为上层的应用程序不是由网络库负责的，它不知道上层的网络程序会绑定一个怎样的上下文对象，提供这个借口方便上层的应用程序根据需要绑定一个每个连接都有的上下文对象；
};
